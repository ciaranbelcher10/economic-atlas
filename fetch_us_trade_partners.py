"""Fetch United States bilateral goods-trade values from UN Comtrade for EVERY partner
Comtrade has data for (not a curated shortlist), and write
data-us-trade-partners.json, consumed by the trade partner map on us.html.
...
own reference docs). us.html's map needs standard ISO codes to match
...
REPORTER = "842"  # United States, Comtrade code (confirmed against official examples/prior research where possible; first live run confirms if not)
...
def main() -> int:
    ...
    with open("data-us-trade-partners.json", "w") as f:
        json.dump(out, f)
    print(f"  ok  comtrade  {len(partners_out)} partners, period {used_period}")
    print(f"\nWrote data-us-trade-partners.json with {len(partners_out)} partners.")
    return 0
