"""Fetch South Africa economic series and write data-za.json.

Run:  FRED_API_KEY=yourkey python3 fetch_za.py
...
- gdp_level (NGDPSAXDCZAQ) and gdp_real (NGDPRSAXDCZAQ): both checked
  ...
  no naming-convention substitution needed.
- fx_to_usd (DEXSFUS): checked directly, live daily H.10 series.
- unemployment: the standard monthly "LRHUTTTT" naming pattern was not
  confirmed to exist for South Africa. What IS confirmed live is
  LRUN64TTZAQ156S (ages 15-64, quarterly, through Q4 2025) -- used here
  as the quarterly substitute, the same pattern Mexico needed.
- bond_yield_10y (IRLTLT01ZAM156N): checked directly, live monthly
  through Feb 2026.
- debt_gdp (GGGDTAZAA188N) / deficit (GGNLBAZAA188N): both checked
  directly, exist, through 2024 -- a year stale, in line with most other
  countries' fiscal data on this site already (this category is
  consistently the slowest-updating one site-wide), used as-is.
- trade_balance (XTNTVA01ZAQ667S): same OECD merchandise-trade quarterly
  convention as every other country -- not individually confirmed.
- cpi: wired in via OECD's live SDMX prices system, the same fix proven
  for six other countries. South Africa, like Brazil and India, is only
  an OECD "key partner" rather than a full member; India has worked
  reliably there, Brazil's result is still pending confirmation as of
  this writing -- reasonable but not certain confidence.
...
# ---- OECD business confidence (South Africa) -- free SDMX API, no key ----
    ...
    for url in OECD_QUERIES:
        ...
        try:
            ...
            for row in csv.DictReader(io.StringIO(r.text)):
                ...
                if low.get("REF_AREA", "ZAF") not in ZA_AREAS:
                    continue
...
# ---- World Bank (South Africa) -- free API, no key ----
WB_URL = ("https://api.worldbank.org/v2/country/ZAF/indicator/"
...
def main() -> int:
            ...
            try:
                ...
                if growth:
                    out["series"]["gdp_growth"] = {
                        "label": "Real GDP growth, QoQ, SA (derived from NGDPRSAXDCZAQ)", "unit": "%",
    ...
    try:
        if key:
            fx_pts = fetch_fred("DEXSFUS", "d", key)
            if fx_pts:
                fx_period, fx_rate = fx_pts[-1]
                out["fx_to_usd"] = {"pair": "ZAR/USD", "rate": fx_rate,
    ...
    with open("data-za.json", "w") as f:
        json.dump(out, f)
    print(f"\nWrote data-za.json with {len(out['series'])} series.")
    if failures:
