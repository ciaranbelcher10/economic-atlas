"""Fetch France bilateral goods-trade values from UN Comtrade for EVERY partner
Comtrade has data for (not a curated shortlist), and write
data-fr-trade-partners.json, consumed by the trade partner map on france.html.
...
own reference docs). france.html's map needs standard ISO codes to match
...
def main() -> int:
    ...
    with open("data-fr-trade-partners.json", "w") as f:
        json.dump(out, f)
    print(f"  ok  comtrade  {len(partners_out)} partners, period {used_period}")
    print(f"\nWrote data-fr-trade-partners.json with {len(partners_out)} partners.")
    return 0
