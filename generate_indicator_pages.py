...
<script>
(function(){{
  // Both failure branches below used to be silent: a missing data file hit an
  // empty catch, and a missing series hit a bare return. Either way the widget
  // sat on its placeholder ellipsis forever on whatever third-party page had
  // embedded it, which reads as a broken widget rather than a data outage.
  function unavailable(){{
    var v = document.getElementById("val");
    if(v) v.textContent = "Data unavailable";
  ...
  fetch("{data_url}").then(function(r){{return r.json();}}).then(function(d){{
    ...
    if(!s || !s.points || !s.points.length){{ unavailable(); return; }}
    var rawPts = s.points;
