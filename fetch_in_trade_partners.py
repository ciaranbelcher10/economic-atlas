"""Fetch India bilateral goods-trade values from UN Comtrade for EVERY partner
Comtrade has data for (not a curated shortlist), and write
data-in-trade-partners.json, consumed by the trade partner map on india.html.
...
own reference docs). india.html's map needs standard ISO codes to match
...
REPORTER = "699"  # India, Comtrade code (confirmed against official examples/prior research where possible; first live run confirms if not)
...
def main() -> int:
    ...
    with open("data-in-trade-partners.json", "w") as f:
        json.dump(out, f)
    print(f"  ok  comtrade  {len(partners_out)} partners, period {used_period}")
    print(f"\nWrote data-in-trade-partners.json with {len(partners_out)} partners.")
    return 0
