...
def main() -> int:
    ...
    extras = [
        ...
        ("cpi_mom", lambda: oecd_prices.mom_points(("NOR",), "M", label="NOR"),
    ...
    # previous run, so a transient failure (e.g. OECD/FRED 429 rate-
    # limiting -- this exact thing hit this country's cpi in the Aug
    # 2026 data-quality sweep, and unlike Poland/Switzerland/Turkey/
    # Chile/Colombia/Indonesia it had no carry-over safety net, so the
    # series went from stale to genuinely gone with no alert) doesn't
    # wipe good data from the live page. Placed BEFORE the "nothing
    # fetched" bailout below (matching Turkey's ordering) so a run
    # where every series fails still gets rescued rather than giving
    # up entirely.
                ...
                for tk in ("trade_balance", "exports", "imports"):
                    if tk in out["series"]:
                        ...
                        if ser["unit"].strip().startswith("$"):
                            ...
                            ser["unit"] = ser["unit"].replace("$", "NOK", 1)
                            ser["label"] = ("Trade balance, goods" if tk == "trade_balance"
                                            else tk.capitalize() + " of goods") + \
                                           " (OECD via FRED, converted to Norwegian kroner at each "\
