"""Fetch Germany bilateral goods-trade values from UN Comtrade for EVERY partner
Comtrade has data for (not a curated shortlist), and write
data-de-trade-partners.json, consumed by the trade partner map on germany.html.
...
own reference docs). germany.html's map needs standard ISO codes to match
...
REPORTER = "276"  # Germany, Comtrade/M49 code (matches ISO 3166-1 numeric; not personally confirmed against a live Comtrade response -- check the [comtrade] log line on first run)
...
def main() -> int:
    ...
    with open("data-de-trade-partners.json", "w") as f:
        json.dump(out, f)
    print(f"  ok  comtrade  {len(partners_out)} partners, period {used_period}")
    print(f"\nWrote data-de-trade-partners.json with {len(partners_out)} partners.")
    return 0
