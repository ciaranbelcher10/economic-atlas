...
def main() -> int:
    ...
    # fx_to_usd: switched (Aug 2026) from COLCCUSMA02STM (OECD monthly)
    ...
    # already accepted for Poland/Turkey/Chile. Needs no FRED_API_KEY
    # (World Bank's API is open), so this now runs unconditionally
    # rather than behind the `if key:` gate the old FRED-based fetch
    # used.
    try:
