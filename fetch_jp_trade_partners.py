"""Fetch Japan bilateral goods-trade values from UN Comtrade for EVERY partner
Comtrade has data for (not a curated shortlist), and write
data-jp-trade-partners.json, consumed by the trade partner map on japan.html.
...
own reference docs). japan.html's map needs standard ISO codes to match
...
REPORTER = "392"  # Japan, Comtrade code (confirmed against official examples/prior research where possible; first live run confirms if not)
...
def main() -> int:
    ...
    with open("data-jp-trade-partners.json", "w") as f:
        json.dump(out, f)
    print(f"  ok  comtrade  {len(partners_out)} partners, period {used_period}")
    print(f"\nWrote data-jp-trade-partners.json with {len(partners_out)} partners.")
    return 0
