...
def transform(points: list, kind: str | None) -> list:
    ...
    # Matched BY PERIOD, not by list position: a hole in the source series
    # used to shift every later comparison by the size of the hole.
    by_period = {p[0]: p[1] for p in points}
    out = []
    for per, val in points:
        prev_per = _period_back_n(per, lag)
        base = by_period.get(prev_per) if prev_per else None
        if not base:
            continue
...
def main() -> int:
    ...
    extras = [
        ...
        ("cpi_mom", lambda: oecd_prices.mom_points(("IND",), "M", label="IND"),
